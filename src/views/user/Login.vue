        ...
        <a-tab-pane key="tab1" :tab="$t('user.login.tab-login-credentials')">
          ...
          <a-form-item>
            <a-input
              size="large"
              type="text"
              :placeholder="$t('user.login.userid.placeholder')"
              v-decorator="[
                'userid',
                {
                  rules: [
...
export default {
  ...
  methods: {
    ...
    handleSubmit (e) {
      ...
      const validateFieldsKey = customActiveKey === 'tab1' ? ['userid', 'password'] : ['mobile', 'captcha']
    ...
    loginSuccess (res) {
      ...
      this.$router.push({ name: 'analysis' }, () => {
        ...
        this.$notification.success({
          message: '환영합니다',
          description: `${timeFix()}，다시 오신 것을 환영합니다`
        })
      })
      */
    ...
    requestFailed (err) {
      ...
      this.$notification['error']({
        message: '에러',
        description: ((err.response || {}).data || {}).message || '요청에 오류가 발생했습니다.，나중에 다시 시도하세요',
        duration: 4
      })
    }
  }
